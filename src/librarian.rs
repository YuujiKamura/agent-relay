...
- WORKING: エージェントがタスクを実行中。入力プロンプトはまだ戻っていない。
- APPROVAL: エージェントが許可を求めて停止している。承認プロンプトが見える。
- DONE: エージェントがタスクを完了して入力プロンプトに戻っている。入力プロンプトの上に作業結果がある。READYとの違い：DONEは作業結果が見える、READYは見えない。
- STOPPED: エージェントが中断・クラッシュした。エラーメッセージがありシェルプロンプトに戻っている。

エージェント固有パターン:
[Codex]
  WORKING: 「Working (30s • esc to interrupt)」「Running ...」「• Ran ...」が見える
  APPROVAL: 「Would you like to run」「Yes, proceed」が見える
  READY: 「› 」プロンプトあり、「Working」「Ran」「Running」なし
  DONE: 「› 」プロンプトあり、上に「Ran」「Running」「Write」「Edit」等の作業ログあり
[Gemini]
  WORKING: 「✦ I'll」「Thinking...」、box UI（╭─...─╮）内にコマンド実行中
  APPROVAL: 「Action Required」「Allow execution」
  READY: 「> Type your message」のみ、作業ログなし
  DONE: 「> Type your message」あり、上に回答文・コード・分析結果がある
[Claude Code]
  WORKING: ツール実行box（╭─ Read/Write/Bash ─╮）が表示中
  APPROVAL: 「Allow once」「Allow always」の選択肢が見える
  READY: 「> 」プロンプトのみ、バージョンバナーが残る程度
  DONE: 「> 」あり、上にツール実行結果・コード変更のログがある

判定の優先順:
1. 許可キーワード（Allow/Approve/Would you like to run） → APPROVAL
2. 作業中キーワード（Working/Running/Thinking/✦/╭─） → WORKING
3. エラー（Error/Failed/crash） → STOPPED
4. プロンプト + 作業結果あり → DONE
5. プロンプト + 作業結果なし → READY
6. エージェントUIなし + シェルプロンプト → IDLE"#;
